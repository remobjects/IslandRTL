...
Public Interface [XMLHttpRequest]
  ...
  Function [send]([parbody] As Dynamic) As Request
  '''<Summary>Sets the value of an HTTP request header. You must call setRequestHeader()after open(), but before send().</Summary>
  Function [setRequestHeader]([parheader] As Dynamic, [parvalue] As Dynamic) As Request
  ...
  '''<Summary>Initializes a request. This method is to be used from native code; to initialize a request from JavaScript code, use open() instead. See the documentation for open().</Summary>
  Function [openRequest]() As Request
End Interface